...
État attendu de `overrides.css` après le Module 4 (~316 lignes). Démontre Grid Layout : `grid-template-areas`, `repeat(auto-fit, minmax())` sur la grille machines, `position: sticky` sur l'en-tête du journal, `display: contents` sur le KPI board. Voir [projet-fil-rouge/checkpoints/04-grid/overrides.css](../projet-fil-rouge/checkpoints/04-grid/overrides.css).
