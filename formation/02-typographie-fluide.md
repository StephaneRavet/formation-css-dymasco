...
État attendu de `overrides.css` après le Module 2 (~200 lignes). Ajoute les tokens fluides `--ml-fs-*` et `--ml-card-width`, applique `clamp()` sur le header title et les KPI, `aspect-ratio: 1` sur le status dot, `100dvh` sur le dashboard. Voir [projet-fil-rouge/checkpoints/02-typography/overrides.css](../projet-fil-rouge/checkpoints/02-typography/overrides.css).
