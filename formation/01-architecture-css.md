...
> ⏱️ **Durée** : 2h00 — **J1 matin<br>**🧭 **Type** : module fondateur (cascade, spécificité, `@layer`, nesting natif, `:where()`, tokens)<br>🎯 **Checkpoint** : `projet-fil-rouge/checkpoints/01-architecture/overrides.css`
---
...
> 🚨 **Plan B — si Apriso injecte son CSS au runtime et vous n'avez pas la main sur le `<link>`**<br>Le Plan A (ci-dessus) repose sur **retirer** le `<link rel="stylesheet" href="apriso-base.css">` du HTML rendu, pour que Apriso ne soit chargé qu'une fois via votre `@import layer(framework)`.<br>Or **côté ScreenBuilder Apriso, ce n'est pas toujours possible** : la plateforme peut injecter ses propres feuilles de style au runtime (via `<head>` généré ASP, via JS, via une config IIS) sans exposer de point d'extension pour les retirer. À vérifier **mission par mission** : ouvrir DevTools → Network, filtrer `.css`, compter les occurrences de `apriso-base.css`. Si vous le voyez chargé en `<link>` que vous ne pouvez pas supprimer côté HTML Layout Editor → **Plan A impossible**.<br>**Conséquence** : Apriso reste **unlayered** côté navigateur, peu importe ce que vous écrivez côté overrides. La règle "unlayered = couche implicite finale = bat tous les layered normaux" s'applique → vos `@layer overrides { … }` perdent silencieusement.
...
```
#### Convention de nommage (à décider en équipe)
...
### 📁 Code de départ
Copiez ce contenu dans `Forge — Pâtes Mamie Lulu.html` :
```html
<!DOCTYPE html>
<!-- saved from url=(0041)file:///Users/sravet/Downloads/index.html -->
<html lang="fr">
...
<head>
  ...
  <title>Forge — Pâtes Mamie Lulu</title>
  <!-- Apriso framework chargé via @import layer(framework) DANS overrides.css.
         Ne PAS ajouter de <link> direct vers apriso-base.css : il serait alors
         chargé en plus comme déclarations unlayered et reprendrait la main sur
         la cascade (cf. Module 1 — règles @layer / unlayered). -->
  <link rel="stylesheet" href="./apriso-base.css">
  <link rel="stylesheet" href="./overrides.css">
</head>

<body>
  <div class="apriso-dashboard">
    <!-- ========== HEADER ========== -->
    <header class="apriso-header">
      <div class="apriso-header__brand">
        <span class="apriso-header__logo">🍝</span>
        <span class="apriso-header__brand-name">Pâtes Mamie Lulu</span>
      </div>

      <h1 class="apriso-header__title">Tableau de bord production</h1>

      <div class="apriso-header__controls">
        <label class="apriso-line-selector">
          <span class="apriso-line-selector__label">Ligne</span>
          <select class="apriso-line-selector__select" name="line">
            <option value="L1">L1 — Pâtes longues</option>
            <option value="L2" selected="">L2 — Pâtes courtes</option>
            <option value="L3">L3 — Lasagnes &amp; spécialités</option>
          </select>
        </label>

        <div class="apriso-connection-status apriso-connection-status--online">
          <span class="apriso-connection-status__dot"></span>
          <span class="apriso-connection-status__label">Connecté</span>
        </div>
      </div>
    </header>

    <!-- ========== KPI BOARD ========== -->
    <section class="apriso-kpi-board" aria-label="Indicateurs clés">
      |  |
|---|

    </section>

    <!-- ========== MAIN CONTENT (machine grid + sidebar event log) ========== -->
    <main class="apriso-main">
      <!-- ----- MACHINE GRID ----- -->
      <section class="apriso-machine-grid" aria-label="Machines de la ligne">
        <article class="apriso-machine-card apriso-machine-card--running">
          <header class="apriso-machine-card__header">
            <h2 class="apriso-machine-card__title">Mélangeur Mama</h2>
            <span class="apriso-machine-card__status">En marche</span>
          </header>
          <dl class="apriso-machine-card__metrics">
            <div class="apriso-machine-card__metric">
              <dt>Cadence</dt>
              <dd>342 kg/h</dd>
            </div>
            <div class="apriso-machine-card__metric">
          ...
          <p class="apriso-machine-card__last-event">Dernière mesure : il y a 8 s</p>
        </article>

        <article class="apriso-machine-card apriso-machine-card--running">
          <header class="apriso-machine-card__header">
            <h2 class="apriso-machine-card__title">Pétrin Papa</h2>
            <span class="apriso-machine-card__status">En marche</span>
          </header>
          <dl class="apriso-machine-card__metrics">
            <div class="apriso-machine-card__metric">
          ...
          <p class="apriso-machine-card__last-event">Dernière mesure : il y a 12 s</p>
        </article>

        <article class="apriso-machine-card apriso-machine-card--alert">
          <header class="apriso-machine-card__header">
            <h2 class="apriso-machine-card__title">Laminoir Léon</h2>
            <span class="apriso-machine-card__status">Alerte</span>
          </header>
          <dl class="apriso-machine-card__metrics">
            <div class="apriso-machine-card__metric">
              <dt>Épaisseur</dt>
          ...
          <p class="apriso-machine-card__last-event">⚠ Dérive d'épaisseur détectée</p>
        </article>

        <article class="apriso-machine-card apriso-machine-card--running">
          <header class="apriso-machine-card__header">
            <h2 class="apriso-machine-card__title">Tagliatelle Tatie</h2>
            <span class="apriso-machine-card__status">En marche</span>
          </header>
          <dl class="apriso-machine-card__metrics">
            <div class="apriso-machine-card__metric">
          ...
          <p class="apriso-machine-card__last-event">Dernière mesure : il y a 4 s</p>
        </article>

        <article class="apriso-machine-card apriso-machine-card--idle">
          <header class="apriso-machine-card__header">
            <h2 class="apriso-machine-card__title">Spaghetti Sergio</h2>
            <span class="apriso-machine-card__status">À l'arrêt</span>
          </header>
          <dl class="apriso-machine-card__metrics">
            <div class="apriso-machine-card__metric">
              <dt>Dernière prod.</dt>
            ...
            <div class="apriso-machine-card__metric">
              <dt>Motif</dt>
              <dd>Changement format</dd>
            </div>
          </dl>
          <p class="apriso-machine-card__last-event">Arrêt planifié — redémarrage 15:00</p>
        </article>

        <article class="apriso-machine-card apriso-machine-card--running">
          <header class="apriso-machine-card__header">
            <h2 class="apriso-machine-card__title">Penne Pépé</h2>
            <span class="apriso-machine-card__status">En marche</span>
          </header>
          <dl class="apriso-machine-card__metrics">
            <div class="apriso-machine-card__metric">
            ...
            <div class="apriso-machine-card__metric">
              <dt>Pression filière</dt>
              <dd>118 bar</dd>
            </div>
          </dl>
          <p class="apriso-machine-card__last-event">Dernière mesure : il y a 2 s</p>
        </article>

        <article class="apriso-machine-card apriso-machine-card--maintenance">
          <header class="apriso-machine-card__header">
            <h2 class="apriso-machine-card__title">Rigatoni Régine</h2>
            <span class="apriso-machine-card__status">Maintenance</span>
          </header>
          <dl class="apriso-machine-card__metrics">
            <div class="apriso-machine-card__metric">
              <dt>Type</dt>
          ...
          <p class="apriso-machine-card__last-event">Intervention en cours — Tech. : J. Moreau</p>
        </article>

        <article class="apriso-machine-card apriso-machine-card--running">
          <header class="apriso-machine-card__header">
            <h2 class="apriso-machine-card__title">Conditionneuse Coco</h2>
            <span class="apriso-machine-card__status">En marche</span>
          </header>
          <dl class="apriso-machine-card__metrics">
            <div class="apriso-machine-card__metric">
          ...
          <p class="apriso-machine-card__last-event">Dernière mesure : il y a 5 s</p>
        </article>
      </section>

      <!-- ----- EVENT LOG (sidebar) ----- -->
      <aside class="apriso-event-log" aria-label="Journal d&#39;événements">
        <header class="apriso-event-log__header">
          <h2 class="apriso-event-log__title">Journal d'événements</h2>
          <span class="apriso-event-log__count">128 événements aujourd'hui</span>
        </header>

        <div class="apriso-event-log__container">
          |  |
|---|
    ...
    <!-- ========== FOOTER ========== -->
    <footer class="apriso-footer">
      <span class="apriso-footer__item">Forge v1.0.0</span>
      <span class="apriso-footer__item">Équipe : Après-midi (14h-22h)</span>
      <span class="apriso-footer__item">Superviseur : C. Bernard</span>
      <span class="apriso-footer__item apriso-footer__item--time">14:42:24</span>
    </footer>
  </div>
...
```
Copiez ce contenu dans  `apriso-base.css` :
```css
/* ============================================================================
   FORGE — APRISO BASE
   ----------------------------------------------------------------------------
   Framework de composants Forge pour DELMIA Apriso.
   Version partagée — NE PAS MODIFIER ce fichier en projet client.
   Toute personnalisation doit se faire dans overrides.css
   ============================================================================
*/
...
/* Hardcoded focus removal — typical of legacy frameworks. */
button:focus,
select:focus,
a:focus {
    outline: none;
...
/* ----------------------------------------------------------------------------
   DASHBOARD — root layout (rigid pixel-based grid)
   ---------------------------------------------------------------------------- */

.apriso-dashboard {
    display: grid;
    grid-template-rows: 64px 96px 1fr 40px;
    grid-template-columns: 1fr;
    height: 100vh;
    width: 1440px;
    /* hardcoded width — typical Apriso target */
    margin: 0 auto;
    background: #ffffff;
...
.apriso-dashboard .apriso-header {
    display: flex;
    align-items: center;
...
.apriso-dashboard .apriso-header .apriso-header__brand {
    display: flex;
    align-items: center;
    gap: 12px;
    font-size: 16px;
...
.apriso-dashboard .apriso-header .apriso-header__logo {
    font-size: 24px;
}

.apriso-dashboard .apriso-header .apriso-header__brand-name {
    letter-spacing: 0.3px;
}

.apriso-dashboard .apriso-header .apriso-header__title {
    font-size: 18px !important;
    font-weight: 500;
    color: #ffffff;
}

.apriso-dashboard .apriso-header .apriso-header__controls {
    display: flex;
    align-items: center;
    gap: 16px;
}

.apriso-line-selector {
    display: flex;
    align-items: center;
    gap: 8px;
    font-size: 13px;
...
.apriso-line-selector .apriso-line-selector__select {
    padding: 6px 10px;
    background: #ffffff;
    color: #1a1a1a;
    border: 1px solid #d0d4da;
    border-radius: 2px;
    width: 220px;
    /* fixed width — text overflow risk */
}

.apriso-connection-status {
    display: flex;
    align-items: center;
    gap: 8px;
    font-size: 13px;
...
.apriso-connection-status .apriso-connection-status__dot {
    width: 8px;
    height: 8px;
    border-radius: 50%;
    background: #6c757d;
...
.apriso-connection-status--online .apriso-connection-status__dot {
    background: #4caf50;
}
...
```javascript
@import url("vendor/some-lib.css") layer(library);
```
...
```
1. **Tests visuels de régression** : Percy/Chromatic ou simple capture avant/après upgrade sur les 5 écrans les plus utilisés. Diff pixel = signal early.
2. **Couche de compat dédiée** : si un upgrade casse beaucoup, intercaler une couche `@layer compat` entre `framework` et `overrides` qui re-mappe les nouveaux noms vers vos anciens overrides :
	```javascript
...
---
## 📝 Récap & checklist
- [ ] Je sais expliquer l'**ordre de cascade** (`!important`, couche, spécificité, ordre)
- [ ] Je sais que pour le CSS **normale** : dernière couche déclarée = gagne, et **unlayered = couche implicite finale = bat tous les layered**
...
- [ ] J'imbrique avec le **nesting natif**, pas besoin de Sass
